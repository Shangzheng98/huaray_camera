...
typedef struct _IMV_FrameInfo
{
	...
	unsigned int			status;					///< \~chinese 数据帧状态										\~english The status of frame
	...
	unsigned int			recvFrameTime;			///< \~chinese 图像在网络传输所用的时间(非GigE相机该值为0)		\~english The time taken for the image to be transmitted over the network(The value is 0 for non-GigE camera)
