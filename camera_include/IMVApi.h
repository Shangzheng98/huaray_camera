...
/// \param nSize [IN] 传输数据块的大小(范围:8-512, 单位:MByte)
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \remarks
/// 1、该接口暂时只有在Linux平台且使用无驱的情况下设置才有效，其他情况下返回IMV_NOT_SUPPORT错误码\n
/// 2、传输数据块数量，范围5 - 256, 默认为64，该值设置过小会影响传输效率；多台相机共同使用时，可以适当减小该值\n
/// 3、传输每个数据块大小，范围8 - 512, 默认为64，单位是MByte
/// \~english
...
/// \param nSize [IN] The size of urb transmitted（range:8-512, unit:MByte）
/// \return Success, return IMV_OK. Failure, return error code
/// \remarks
/// 1、The interface is only supported on the Linux platform without USB driver. In other cases, it returns IMV_NOT_SUPPORT error code.\n
/// 2、The number of urb transmitted, the range is 5 - 256, and the default is 64. If setting the value too small will affect the transmission efficienc;
/// when multiple cameras are used together, the value can be appropriately reduced.\n
/// 3、The size of each urb transmitted, the range is 8 - 512, the default is 64, the unit is MByte.
